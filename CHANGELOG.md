...
- **Selecting across a list, a heading or an image no longer makes the text jump under the pointer**: the note stays formatted while you drag, and the syntax of everything selected shows when you let go — so you see exactly what will be copied. A selection that starts or ends at a heading, a list item or bold text takes its `#`, bullet or `**` along.
