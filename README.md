...
- **Hold a card and the actions open around your finger** — complete, pull into today, pin, move, and a ⋮ with the rest — then drag to the one you want and let go. Letting go anywhere else does nothing. Note cards and the sidebar's spaces have their own set; on the computer, a note card's ⋮ opens the same ring with a click.
