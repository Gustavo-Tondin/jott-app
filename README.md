...
- **Move a card without dragging it**: `Alt+↑` and `Alt+↓` on the focused task, and a screen reader is told where it landed. Drag one to the top or the bottom of a long list and the list scrolls along with it.
- **Hold a card and its actions open beside your finger** — pin, move, edit, reorder, and a ⋮ with the rest, the same five on a task and on a note — as a column of squares; slide onto the one you want, which names itself as you reach it, and let go. Letting go anywhere else does nothing. Completing a task and pulling it into today are the swipes. On the computer the **right button** opens the same column at the pointer — on a task, a note or a space in the sidebar — and a card's ⋮ is the whole menu.
...
- **The sidebar colours itself**: every space and group takes the next colour of the rainbow, in the order they sit in, and moving one deals the column again. Pick a colour for one by hand and the column keeps exactly the colours it is wearing, yours among them — from then on what you make takes the next colour and nothing moves on its own. The tick is in the menu of the empty sidebar (right-click, or hold on a phone).
...
- The Home's **+** opens two buttons, Task and Note (a blank note opens with the cursor in the body). On a phone every space has the same **+**: one tap makes a blank note in a notes space, or opens the task bar in a list. On a phone the Home's head folds: drag it up for one line, down for the week and the day's summary; scroll, and the canvas slides up over it, takes the title into its own colour as it passes, and rests either with the head open or filling the screen.
- **The Home is the day**: a week of days across the top, today lit — drag the strip sideways, or use the arrows, for the weeks around it. You pull in what you want to face today instead of staring at the whole list; pick a day ahead and plan it the same way (a dated task shows on its day on its own); pick a day gone by and read what happened on it. Suggestions are ordered by urgency, never auto-selected. Today's notes — the ones written or edited today — come from **every** notepad, each card marked with its space's colour.
...
- `[[Note]]` links between notes and `[[/file.pdf]]` references to files, both with autocompletion; colour or image banners, set from the note's title or from its card; pinning; a **reorder mode** (the ring's "Reorder", or the notepad's ⋮) where a tap marks cards, a hold carries them — one, or every marked one as a pile — and a bar at the foot moves or deletes the marked. Outside it a note is never dragged by accident. A card's actions rename the note without opening it, and a note you never named shows its text instead of the app's "New note".
...
- **Outside changes are detected** — edit a file in another editor, or let Syncthing/Drive sync it, and the app follows, the open note included. Typing while the other version lands keeps both: yours on screen, theirs as a conflict copy beside the note. **What two devices did apart is put back together**: different tasks of the same list, different parts of the same note, the day they arranged, the days they planned. A copy is left only where both changed the same thing, the banner then says what differs, and whichever version goes, goes to the Trash.
