...
- **Hold a card and its actions open beside your finger** — complete, pull into today, pin, move, and a ⋮ with the rest — as a column of squares; slide onto the one you want, which names itself as you reach it, and let go. Letting go anywhere else does nothing. Note cards and the sidebar's spaces have their own set; on the computer, a note card's ⋮ opens the same column with a click.
