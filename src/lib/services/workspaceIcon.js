...
//
// A workspace has one function, and the icon says which at a glance — the
...
/// The icon of each workspace type, when the user has not chosen one.
...
/// The icon for a workspace — its own, or the one its type wears.
export function workspaceIcon(workspace) {
  return workspace?.icon || TYPE_ICONS[workspace?.kind] || DEFAULT_ICON;
}
