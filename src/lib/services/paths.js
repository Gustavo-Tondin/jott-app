...
/// The file name every tasks workspace's single list carries (core's
...
/// file name — hyphenated, lowercase, the same in every workspace — and it was
...
/// How a list is named in a picker: the READABLE ADDRESS of the workspace it
/// lives in — `Design/Tasks`, or just `Mercado` when the workspace is loose.
///
/// The stem used to be appended to it, which is how the picker came to say
/// "Tasks/Work" for a workspace the user had renamed to Tasks: the file kept
/// the old name, and the label showed both halves of the disagreement. A tasks
/// workspace is ONE list whose file name is now fixed, so the stem carries no
...
export function listLabel(entry) {
  const where = entry?.workspace ?? "";
  const stem = entry?.name ?? listName(entry?.path);
  // Loose enough to have no workspace at all (a hand-made address): then the
